          ...
          <div class="grid grid-cols-2 gap-16 items-center">
            ...
            <div class="animate-fade-in-up">
              ...
              <div class="flex items-center gap-4 mb-8">
                <a routerLink="/recherche" class="px-8 py-4 bg-accent text-white rounded-lg font-semibold shadow-lg shadow-accent/25 hover:bg-accent/90 transition-all hover:scale-105 active:scale-95">
                  Déposer mon projet
                </a>
            ...
            <!-- Right Illustration -->
            <div class="animate-fade-in-scale delay-200 relative">
              <div class="relative w-full aspect-square">
                <!-- Central node -->
                <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-32 h-32 bg-accent rounded-full flex items-center justify-center shadow-2xl shadow-accent/30">
                ...
                @for (node of orbitNodes; track $index) {
                  <div
                    class="absolute top-1/2 left-1/2 w-20 h-20 bg-white border-4 border-accent/30 rounded-full flex items-center justify-center shadow-xl animate-scale-in"
                    [style.transform]="node.transform"
                    [class]="'delay-' + node.delay"
                  >
      ...
      <section class="py-20 px-8 bg-white">
        <div class="max-w-[1400px] mx-auto">
          <div class="text-center mb-16 animate-fade-in-up">
            ...
            <p class="text-lg text-muted-foreground">Trouvez la solution adaptée à vos besoins</p>
          </div>
      ...
      <section class="py-20 px-8 bg-white">
        <div class="max-w-[1400px] mx-auto">
          <div class="text-center mb-16 animate-fade-in-up">
            ...
            <p class="text-lg text-muted-foreground">Choisissez la formule qui vous convient</p>
          </div>

          <div class="grid grid-cols-3 gap-8 mb-12">
            @for (plan of pricingPreview; track plan.name) {
              <div
                class="bg-card rounded-2xl p-8 border-2 shadow-md text-center animate-fade-in-up transition-transform hover:-translate-y-1 duration-300"
              ...
              >
                ...
                <a routerLink="/tarifs"
                  [ngClass]="plan.popular ? 'block w-full py-3 rounded-lg font-semibold transition-colors cursor-pointer bg-accent text-white' : 'block w-full py-3 rounded-lg font-semibold transition-colors cursor-pointer border-2 border-border'"
                >En savoir plus</a>
              </div>
            }
          </div>
...
export class LandingComponent {
  readonly trustBadges = ['Gratuit pour commencer', 'Matching IA', '+200 mises en relation réussies'];

  readonly steps = [
    { emoji: '👤', title: 'Créez votre profil', description: 'Décrivez votre projet et vos besoins en quelques minutes' },
    { emoji: '✨', title: 'Notre IA trouve vos matches', description: 'Algorithme intelligent pour des suggestions pertinentes' },
    { emoji: '🤝', title: 'Entrez en contact', description: 'Échangez directement avec vos futurs associés' },
  ];

  readonly useCases = [
    { icon: '🤝', title: 'Je cherche un associé ou co-fondateur', description: 'Trouvez la personne qui partagera votre vision et complétera vos compétences pour bâtir ensemble.' },
    { icon: '💼', title: 'Je cherche un talent (dev, commercial, expert)', description: 'Recrutez des profils qualifiés prêts à rejoindre votre aventure entrepreneuriale.' },
    { icon: '🏢', title: "J'achète ou je vends une entreprise", description: 'Accédez à notre marketplace M&A pour des transactions sécurisées et confidentielles.' },
  ];
  ...
  readonly testimonials = [
    { name: 'Sophie Martin', role: 'CEO, TechFlow', avatar: 'SM', quote: "J'ai trouvé mon CTO en 2 semaines grâce à GoFounders. Le matching IA est bluffant !" },
    { name: 'Thomas Dupont', role: 'Fondateur, GreenBox', avatar: 'TD', quote: 'Une plateforme qui comprend vraiment les besoins des entrepreneurs. Recommande à 100%.' },
    { name: 'Claire Rousseau', role: 'Co-fondatrice, Educare', avatar: 'CR', quote: 'Le processus est fluide et les profils sont de grande qualité. Excellent service.' },
  ];
  ...
  readonly orbitNodes = [0, 1, 2, 3, 4].map(i => {
    const angle = (i * 360) / 5;
    const radius = 180;
  ...
  readonly connectionLines = [0, 1, 2, 3, 4].map(i => {
    const angle = (i * 360) / 5;
    return {

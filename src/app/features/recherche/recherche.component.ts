...
type ProfileRole = 'entrepreneur' | 'talent';
        ...
        <div class="flex gap-8">
          ...
          <aside class="w-[280px] flex-shrink-0">
            <div class="bg-secondary rounded-2xl p-6 sticky top-28">
              ...
              <!-- Demo role switcher -->
              <div class="mb-6 p-3 bg-white border-2 border-accent/30 rounded-xl">
                <p class="text-xs font-bold text-accent mb-2">Démo — vous êtes :</p>
                <div class="flex gap-2">
                  <button
                    (click)="currentUserType.set('entrepreneur')"
                    [ngClass]="currentUserType() === 'entrepreneur' ? 'flex-1 py-1.5 rounded-lg text-xs font-semibold bg-accent text-white' : 'flex-1 py-1.5 rounded-lg text-xs font-semibold bg-secondary text-muted-foreground'"
                  >Porteur</button>
                  <button
                    (click)="currentUserType.set('talent')"
                    [ngClass]="currentUserType() === 'talent' ? 'flex-1 py-1.5 rounded-lg text-xs font-semibold bg-accent text-white' : 'flex-1 py-1.5 rounded-lg text-xs font-semibold bg-secondary text-muted-foreground'"
                  >Talent</button>
                </div>
              </div>

              <div class="space-y-6">
                <!-- Type de profil visible (info only) -->
                <div>
                  <label class="block text-sm font-bold mb-2">Vous voyez</label>
                  <p class="text-sm text-muted-foreground">
                    @if (currentUserType() === 'entrepreneur') {
                      Talents et experts disponibles
                    } @else {
          ...
          <div class="flex-1">
            ...
            <div class="flex items-center justify-between mb-8">
              <div class="text-lg">
                <span class="font-bold text-primary">{{ visibleProfiles().length }}</span>
                <span class="text-muted-foreground"> {{ currentUserType() === 'entrepreneur' ? 'talents disponibles' : 'projets disponibles' }}</span>
              </div>
            ...
            <div class="bg-gradient-to-r from-accent to-primary rounded-2xl p-6 mb-8 text-white shadow-lg animate-fade-in-up">
              <div class="flex items-center gap-2 mb-4">
                ...
                <h2 class="text-xl font-bold">
                  @if (currentUserType() === 'entrepreneur') {
                    ✨ 8 talents correspondent particulièrement à votre projet
                  } @else {
                    ✨ 8 projets correspondent particulièrement à votre profil
                  }
                </h2>
              </div>
            ...
            <div class="grid grid-cols-3 gap-6 relative">
              @for (profile of visibleProfiles(); track $index) {
                <div
...
export class RechercheComponent {
  currentUserType = signal<ProfileRole>('entrepreneur');
  ...
  readonly allProfiles: Profile[] = [
    { name: 'Sophie Martin', role: 'CTO & Co-fondatrice', profileType: 'talent', location: 'Paris', stage: 'MVP', bio: "Passionnée par l'IA et la santé tech. Cherche co-fondateur business pour scaler une solution SaaS.", skills: ['React', 'Python', 'IA'], match: 95, avatar: 'SM' },
    { name: 'Thomas Dupont', role: 'Développeur Full-Stack', profileType: 'talent', location: 'Lyon', stage: 'Idée', bio: 'Expert e-commerce et marketplaces. Disponible pour rejoindre un projet ambitieux.', skills: ['Node.js', 'AWS', 'DevOps'], match: 92, avatar: 'TD' },
    { name: 'Claire Bernard', role: 'Product Manager', profileType: 'talent', location: 'Bordeaux', stage: 'En croissance', bio: 'Ex-Google, spécialiste produit B2B SaaS. Envie de rejoindre une aventure early-stage.', skills: ['Product', 'UX', 'Analytics'], match: 89, avatar: 'CB' },
    { name: 'Marc Laurent', role: 'Commercial Senior', profileType: 'talent', location: 'Marseille', stage: 'Établi', bio: "Expert vente B2B avec 10 ans d'expérience. Disponible pour un projet à fort potentiel.", skills: ['Sales', 'Négociation', 'B2B'], match: 87, avatar: 'ML' },
    { name: 'Emma Rousseau', role: 'Designer UI/UX', profileType: 'talent', location: 'Toulouse', stage: 'MVP', bio: 'Créative et data-driven. Cherche startup tech pour apporter une vision design forte.', skills: ['Figma', 'Design System', 'Branding'], match: 84, avatar: 'ER' },
    { name: 'Léa Fontaine', role: 'Fondatrice SaaS RH', profileType: 'entrepreneur', location: 'Paris', stage: 'MVP', bio: "Projet de plateforme RH pour les PME. Cherche un CTO pour co-fonder.", skills: ['RH', 'B2B', 'SaaS'], match: 91, avatar: 'LF' },
    { name: 'Antoine Girard', role: 'Porteur de projet Fintech', profileType: 'entrepreneur', location: 'Lyon', stage: 'Idée', bio: "Idée validée dans la Fintech B2C. Cherche développeur mobile et expert conformité.", skills: ['Fintech', 'Mobile', 'Réglementation'], match: 86, avatar: 'AG' },
    { name: 'Julie Moreau', role: 'Data Scientist', profileType: 'talent', location: 'Lille', stage: 'Idée', bio: 'PhD en Machine Learning. Envie de créer un produit IA avec impact social.', skills: ['Python', 'ML', 'Data Viz'], match: 79, avatar: 'JM' },
    { name: 'Camille Renard', role: 'Fondatrice EdTech', profileType: 'entrepreneur', location: 'Nantes', stage: 'En croissance', bio: "Plateforme de cours en ligne pour lycéens. Cherche Growth Marketer et développeur.", skills: ['EdTech', 'Marketing', 'B2C'], match: 83, avatar: 'CR' },
  ];

  readonly visibleProfiles = computed(() => {
    const targetType: ProfileRole = this.currentUserType() === 'entrepreneur' ? 'talent' : 'entrepreneur';
    return this.allProfiles.filter(p => p.profileType === targetType);
  });

  readonly highlightedProfiles = computed(() =>
    this.visibleProfiles().filter(p => p.match >= 85)
  );

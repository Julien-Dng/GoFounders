          ...
          <div class="col-span-8">
            ...
            <div class="bg-white rounded-2xl border-2 border-border shadow-xl p-8 mb-6 animate-fade-in-up">
              <div class="flex items-start gap-6">
                <div class="relative">
                  <div class="w-32 h-32 bg-gradient-to-br from-accent to-primary rounded-2xl flex items-center justify-center text-white text-4xl font-bold shadow-lg">{{ profileInitials() }}</div>
                ...
                <div class="flex-1">
                  <div class="flex items-start justify-between mb-3">
                    <div>
                      <div class="flex items-center gap-3 mb-2">
                        ...
                        <span class="px-3 py-1 bg-accent text-white rounded-full text-sm font-semibold">{{ profileRole() }}</span>
                      </div>
                  ...
                  <div class="flex items-center gap-3 mt-6">
                    @if (isOwnProfile()) {
                      <a routerLink="/profil/modifier" class="flex items-center gap-2 px-6 py-3 bg-accent text-white rounded-lg font-semibold shadow-lg shadow-accent/25 hover:bg-accent/90 transition-all hover:scale-105 active:scale-95">
...
export class ProfilComponent {
  ...
  private readonly routeId = toSignal(
    this.route.params.pipe(map((p: Record<string, string>) => p['id'] ?? '')),
    { initialValue: '' }
  );
  ...
  readonly profileName = computed(() =>
    this.isOwnProfile() ? (this.auth.currentUser()?.displayName ?? '') : 'Sophie Bernard'
  );

  readonly profileInitials = computed(() =>
    this.isOwnProfile() ? this.auth.initials() : 'SB'
  );

  readonly profileLocation = computed(() =>
    this.isOwnProfile() ? (this.auth.currentUser()?.location ?? 'Paris, France') : 'Paris, France'
  );

  readonly profileRole = computed(() =>
    this.isOwnProfile() ? (this.auth.currentUser()?.profileType === 'talent' ? 'Talent / Expert' : 'Porteur de projet') : 'CEO & Fondatrice'
  );
